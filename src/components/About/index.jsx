    ...
    return (
      <div className={"about-bitcoin"}>
        ...
        <p className='bitcoin-text'>
          ...
          <span className='purple-text'>${numberWithCommas(`${data.price}`)}</span> with a 24-hour
          trading volume of{' '}
          <span className='purple-text'>${numberWithCommas(`${data.volume_24h}`)} </span>. BTC price
          is down{' '}
          <span className='purple-text'>{numberWithCommas(`${data.percent_change_24h}`)}%</span> in
          the last 24 hours. 
        </p>
      </div>
    );
